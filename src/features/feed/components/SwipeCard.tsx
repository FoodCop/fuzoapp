...
export const SwipeCard = forwardRef<SwipeCardRef, { children: React.ReactNode; onSwipe: (dir: SwipeDirection) => void; active: boolean }>(
  ({ children, onSwipe, active }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      swipe: (dir: SwipeDirection) => {
        ...
        if (dir === 'left') targetOffset = { x: -distance, y: 0 };
        else if (dir === 'right') targetOffset = { x: distance, y: 0 };
        else if (dir === 'up') targetOffset = { x: 0, y: -distance };
        else if (dir === 'down') targetOffset = { x: 0, y: distance };
