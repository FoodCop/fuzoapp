...
export const SocialButton = ({ provider, onClick, isLoading, disabled }: SocialButtonProps) => {
  ...
  return (
    <motion.button
      ...
      className={`relative w-full py-5 rounded-[2.5rem] border-2 flex items-center justify-center gap-3 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed group ${config.bgColor} ${config.borderColor} ${config.textColor}`}
    >
      {isLoading ? (
